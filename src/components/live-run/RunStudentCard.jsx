...
export default function RunStudentCard({ student, participant, totalLaps, grade, passThreshold = 55, attendanceStatus, onAttendanceChange, onFinish, onNotParticipate, onUndo, onSetLaps }) {
  ...
  if (status === 'not_participated' || status === 'not_completed') {
    return (
      <div className="rounded-2xl border bg-muted/40 p-3" dir="rtl">
  ...
  if (status === 'finished') {
    return (
      <div className="rounded-2xl border border-green-200 bg-green-50 dark:bg-green-950/20 dark:border-green-900 p-3" dir="rtl">
        <div className="flex items-center gap-2">
          <div className="flex-1 min-w-0 text-right">
  ...
  return (
    <div className="rounded-2xl border bg-card p-3" dir="rtl">
      <div className="flex items-center gap-2">
        <div className="flex-1 min-w-0 text-right">
          <h3 className="font-extrabold text-[15px] truncate">{name}</h3>
          <div className="flex items-center gap-1.5 mt-2 justify-start">
