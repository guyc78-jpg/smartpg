...
export default function EducatorContactButton({ cls, students, isAdmin, onSaveContacts }) {
  ...
  return (
    <>
      <button type="button" onClick={handleClick} aria-label={isAdmin ? `ניהול מחנכים לכיתה ${cls.name}` : `הודעה למחנכי הכיתה ${cls.name}`} title={isAdmin ? 'ניהול מחנכים' : 'הודעת WhatsApp למחנכ/ת'} className="relative h-8 w-8 shrink-0 flex items-center justify-center rounded-lg text-muted-foreground hover:text-primary hover:bg-primary/10 transition-colors">
