...
export default function AddClassDialog({ open, onOpenChange, onAdd, defaultGenderTrack = 'boys' }) {
  ...
  const handleSave = async () => {
    ...
    await onAdd(trimmed, gradeLevel, genderTrack, students);
    setName('');
    setGradeLevel('ז');
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[360px] rounded-2xl shadow-lg p-0 overflow-hidden" dir="rtl">
        ...
        <div className="px-5 pb-5 space-y-3">
          <div className="flex gap-2 items-end flex-wrap">
            <div className="flex-1 space-y-1 min-w-[120px]">
              ...
              <Input value={name} onChange={e => setName(e.target.value)} placeholder="לדוגמה: ז׳ 1" autoFocus className="h-9 text-sm" />
            </div>
            ...
            <div className="space-y-1">
              <Label className="text-[11px] text-muted-foreground">מסלול</Label>
          ...
          <div className="space-y-1">
            ...
            {studentNames && (
              <p className="text-[10px] text-muted-foreground/60">
                {studentNames.split('\n').filter(s => s.trim()).length} תלמידים
              </p>
            )}
          </div>
          ...
          <div className="flex gap-2 pt-1">
            ...
            <Button variant="outline" onClick={() => onOpenChange(false)} className="h-10 rounded-xl btn-3d font-semibold text-sm px-5">
              ביטול
            </Button>
          </div>
        </div>
